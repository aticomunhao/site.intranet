...
if($Acao=="buscadados"){
    ...
    $Erro = 0;
    $rs = pg_query($Conec, "SELECT num_ap, localap FROM ".$xProj.".controle_ar4 WHERE id = $Cod");
    $row = pg_num_rows($rs);
    if(!$rs){
        $Erro = 1;
...
if($Acao=="salvadados"){
    ...
    $Erro = 0;
    $rs0 = pg_query($Conec, "SELECT MAX(num_ap) FROM ".$xProj.".controle_ar4");
    $tbl0 = pg_fetch_row($rs0);
        $Codigo = $tbl0[0];
        $Prox = ($Codigo+1);

//    $Prox = ($tbl0[0]+1);
    $rs = pg_query($Conec, "INSERT INTO ".$xProj.".controle_ar4 (num_ap, localap, empresa_id) VALUES ($Prox, '$Local', $Empresa)");
...
if($Acao=="buscanumero"){
    $Erro = 0;
    $Prox = 1;
    $rs = pg_query($Conec, "SELECT MAX(num_ap) FROM ".$xProj.".controle_ar4");
    $tbl = pg_fetch_row($rs);
    $Prox = ($tbl[0]+1);
...
if($Acao=="buscadata"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo'); // id de visitas_ar4
    $Erro = 0;
    //controle_id, datavis, , , , tipovis, empresa_id,  , nometec, , , usuins, datains, ativo
    $rs = pg_query($Conec, "SELECT num_ap, localap, to_char(datavis, 'DD/MM/YYYY'), nometec, ".$xProj.".visitas_ar4.empresa_id, tipovis, 
    ...
    FROM ".$xProj.".controle_ar4 INNER JOIN ".$xProj.".visitas_ar4 ON ".$xProj.".controle_ar4.id = ".$xProj.".visitas_ar4.controle_id 
    WHERE ".$xProj.".visitas_ar4.id = $Cod");
    $row = pg_num_rows($rs);
    if(!$rs){
        $Erro = 1;
        $var = array("coderro"=>$Erro);
...
if($Acao=="salvadatainsprevent"){
    ...
    $Erro = 0;
    if($InsEdit == 0){ // inserindo: é o id de controle_ar4
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".visitas_ar4");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".visitas_ar4 (id, controle_id, datavis, nometec, usuins, datains, empresa_id, ativo, tipovis, acompanh) 
        VALUES ($CodigoNovo, $Cod, '$Data', '$Nome', ".$_SESSION["usuarioID"].", NOW(), $Empresa, 1, $Tipo, '$AcompPrev')");
    }else{ // salvando: é o id de visitar_ar
        $rs = pg_query($Conec, "UPDATE ".$xProj.".visitas_ar4 SET datavis = '$Data', nometec = '$Nome', dataedit = NOW(), usuedit = ".$_SESSION["usuarioID"].", acompanh = '$AcompPrev' WHERE id = $Cod ");
    }
    if(!$rs){
        $Erro = 1;
    }
...
if($Acao=="salvamanutcorret"){
    ...
    if($InsEdit == 0){ // inserindo: é o id de controle_ar4
        ...
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".visitas_ar4 (id, controle_id, datavis, acionam, atendim, conclus, tipovis, empresa_id, contato, acompanh, nometec, defeito, diagtec, svcrealizado, usuins, datains, ativo) 
       VALUES ($CodigoNovo, $Cod, '$DataVis', '$Acionam', '$Atendim', '$Conclus', $Tipo, $Empresa, '$NomeContactado', '$NomeAcompanhante', '$NomeTecnico', '$Defeito', '$Diagnostico', '$SvcRealiz', ".$_SESSION["usuarioID"].", NOW(), 1)");
        ...
        $rs = pg_query($Conec, "UPDATE ".$xProj.".visitas_ar4 SET datavis = '$DataVis', acionam = '$Acionam', atendim = '$Atendim', conclus = '$Conclus', 
...
if($Acao=="salvadataedit"){
    ...
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".visitas_ar4 SET datavis = '$Data', nometec = '$Nome', empresa_id = $Empresa, tipovis = $Tipo, ativo = 1, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod");
...
if($Acao=="buscalocal"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "SELECT num_ap, localap, empresa_id FROM ".$xProj.".controle_ar4 WHERE id = $Cod");
    $row = pg_num_rows($rs);
    if(!$rs){
        $Erro = 1;
        $var = array("coderro"=>$Erro);
...
if($Acao=="salvalocal"){
    ...
    $Empresa = (int) filter_input(INPUT_GET, 'empresa');
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".controle_ar4 SET localap = '$Local', empresa_id = $Empresa, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod");
    if(!$rs){
        $Erro = 1;
    }
    $var = array("coderro"=>$Erro);
...
if($Acao=="apagadata"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".visitas_ar4 SET ativo = 0, usudel = ".$_SESSION["usuarioID"].", datadel = NOW() WHERE id = $Cod");
    if(!$rs){
        $Erro = 1;
    }
    $var = array("coderro"=>$Erro);
