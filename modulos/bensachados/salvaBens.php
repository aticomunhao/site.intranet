...
if($Acao=="buscaBem"){
    ...
    TO_CHAR(AGE(CURRENT_DATE, datareceb), 'MM') AS intervalo, destinonodestino, setordestino, nomerecebeudestino FROM ".$xProj.".bensachados WHERE id = $Codigo ");
    ...
    if(!$rs1){
        ...
        $NomeUsuIns = $tbl2[0];
        $var = array("coderro"=>$Erro, "datareg"=>$tbl1[0], "dataachou"=>$tbl1[1], "descdobem"=>nl2br($tbl1[2]), "localachou"=>$tbl1[3], "nomeachou"=>$tbl1[4], "telefachou"=>$tbl1[5], "numprocesso"=>$tbl1[6], "codusuins"=>$CodUsuIns, "nomeusuins"=>$NomeUsuIns, "intervalo"=>$tbl1[8], "destino"=>$tbl1[9], "setordestino"=>$tbl1[10], "nomerecebeu"=>$tbl1[11]);
    }
    $responseText = json_encode($var);
    echo $responseText;
}
