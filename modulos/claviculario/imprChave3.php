...
if(isset($_REQUEST["acao"])){
    ...
    $pdf->SetTitle('Resumo Chaves Lacradas', $isUTF8=TRUE);
    //Monta o arquivo pdf        
...
//    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
    ...
    $pdf->MultiCell(0, 3, "Claviculário de Chaves Lacradas", 0, 'C', false);
    ...
    if($Acao == "listamesChaves" || $Acao == "listaanoChaves"){
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                if($Acao == "listamesChaves"){
                    $rs1 = pg_query($Conec, "SELECT TO_CHAR(datasaida, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datavolta, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datasaida, 'YYYY'), TO_CHAR(datavolta, 'YYYY'), usuretira, usudevolve, TO_CHAR(datavolta - datasaida, 'DD HH24:MI'), TO_CHAR(datavolta - datasaida, 'DD'), TO_CHAR(CURRENT_DATE - datasaida, 'DD'), telef FROM ".$xProj.".chaves3_ctl 
    ...
    if($Acao == "listamesChaves"){
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                $rs1 = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('MONTH', datasaida) = '$Mes' And DATE_PART('YEAR', datasaida) = '$Ano'");
                $row1 = pg_num_rows($rs1);
    ...
    if($Acao == "listaanoChaves"){
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                $rs1 = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('YEAR', datasaida) = '$Ano'");
                $row1 = pg_num_rows($rs1);
                $pdf->Cell(20, 5, $row1, 0, 0, 'R');
                $pdf->Cell(8, 5, "", 0, 0, 'R'); //espaço
                $pdf->Cell(80, 5, $tbl0[3], 0, 1, 'L');
