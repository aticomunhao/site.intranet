...
if(isset($_REQUEST["acao"])){
    ...
    if($Acao == "listamesChaves" || $Acao == "listaanoChaves"){
        ...
        $pdf->SetTitle('Claviculário Lacrado', $isUTF8=TRUE);
        if($Dom != "" && $Dom != "NULL"){
...
//        $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
        ...
        $pdf->MultiCell(0, 3, "Claviculário Lacrado", 0, 'C', false);
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                $rsTot = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And ativo = 1");
                ...
                if($Acao == "listamesChaves"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('MONTH', datasaida) = '$Mes' And DATE_PART('YEAR', datasaida) = '$Ano' And ativo = 1");
                ...
                if($Acao == "listaanoChaves"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('YEAR', datasaida) = '$Ano' And ativo = 1");
                    $rowCont = pg_num_rows($rsCont);
                }
            ...
            $pdf->SetX(50);
            $pdf->Cell(40, 5, 'Nenhum chave encontrado.', 0, 1, 'L');
        }
    $pdf->Output();
    }
...
//--------
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                if($Acao == "listamesChavesSoMovimentados"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('MONTH', datasaida) = '$Mes' And DATE_PART('YEAR', datasaida) = '$Ano'");
                    $rowCont = pg_num_rows($rsCont);
                }
                if($Acao == "listaanoChavesSoMovimentados"){
                    $rs1 = pg_query($Conec, "SELECT TO_CHAR(datasaida, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datavolta, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datasaida, 'YYYY'), TO_CHAR(datavolta, 'YYYY'), usuretira, usudevolve, TO_CHAR(datavolta - datasaida, 'DD HH24:MI'), TO_CHAR(datavolta - datasaida, 'DD'), TO_CHAR(CURRENT_DATE - datasaida, 'DD'), telef 
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves3_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('YEAR', datasaida) = '$Ano'");
                    $rowCont = pg_num_rows($rsCont);
                }
    ...
    //    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
        ...
        $pdf->SetFont('Arial', '' , 10);
        $pdf->SetTextColor(25, 25, 112);
        $pdf->MultiCell(0, 3, "Claviculário das Chaves Lacradas", 0, 'C', false);
