...
?>
    <div style="text-align: center; margin: 5px;">
        <h5>Agenda</h5>
        <?php 
        $rs = pg_query($Conec, "SELECT ".$xProj.".chaves3_agd.id, chaves_id, chavenum, chavenumcompl, chavesala, TO_CHAR(datasaida, 'DD/MM/YYYY'), usuretira, cpfretira, telef, chavelocal, datasaida 
        FROM ".$xProj.".chaves3_agd INNER JOIN ".$xProj.".chaves3 ON ".$xProj.".chaves3_agd.chaves_id = ".$xProj.".chaves3.id 
        WHERE ".$xProj.".chaves3.ativo = 1 And ".$xProj.".chaves3_agd.ativo = 1 ORDER BY datasaida ");
        $row = pg_num_rows($rs);
        $Hoje = date('Y/m/d');
        if($row > 0){
            while($tbl = pg_fetch_row($rs)){
                $CodAg = $tbl[0];
                $CodChaves = $tbl[1]; // chaves_id de chaves3_ctl
                ?>
                <div style="border: 2px solid #CFB53B; border-radius: 8px; padding: 5px;">
                    <table style="margin: 0 auto; width:95%">
                        <tr>          
                            <td colspan="2"><div class="quadrlista" style="border-color: #E90074; font-size: 120%;"> <?php echo str_pad($tbl[2], 3, 0, STR_PAD_LEFT)." ".$tbl[3]; ?></div>
