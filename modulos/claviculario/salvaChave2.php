...
if(isset($_REQUEST["acao"])){
    ...
    if($Acao == "retornoChave1"){
        ...
        $rs0 = pg_query($Conec, "SELECT chavenum, chavenumcompl FROM ".$xProj.".chaves2 WHERE id = $Cod");
    ...
    if($Acao == "buscaChaveAgenda"){
        ...
        $rs = pg_query($Conec, "SELECT chavenum, chavenumcompl, chavelocal, chavesala, chaveobs, presente FROM ".$xProj.".chaves2 WHERE id = $Cod");
        $row = pg_num_rows($rs);
        if($row > 0){
            $tbl = pg_fetch_row($rs);
            $ChaveNum = str_pad($tbl[0], 3, 0, STR_PAD_LEFT);
