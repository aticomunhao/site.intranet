...
//        $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
        ...
        $rs0 = pg_query($Conec, "SELECT ".$xProj.".chaves2.id, chavenum, chavenumcompl, chavelocal, chavesala, chaveobs, presente, chavecompl 
        FROM ".$xProj.".chaves2  
        ...
        if($row0 > 0){
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                $rsTot = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves2_ctl WHERE ativo = 1 And chaves_id = $Cod And ativo = 1");
                ...
                if($Acao == "listamesChaves"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves2_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('MONTH', datasaida) = '$Mes' And DATE_PART('YEAR', datasaida) = '$Ano' And ativo = 1");
                ...
                if($Acao == "listaanoChaves"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves2_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('YEAR', datasaida) = '$Ano' And ativo = 1");
                    $rowCont = pg_num_rows($rsCont);
                }
...
//                    $pdf->Cell(25, 5, "Ret: ".$rowCont."/".$rowTot, 0, 0, 'L'); // núm de retiradas de cada chave, mensal ou anual
            ...
            $pdf->SetX(50);
            $pdf->Cell(40, 5, 'Nenhuma chave encontrado.', 0, 1, 'L');
        }
    $pdf->Output();
    }
...
//--------
    ...
    //    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
        ...
        WHERE ".$xProj.".chaves2.ativo = 1 And chavenum != 0 And ".$xProj.".chaves2_ctl.ativo = 1 
        GROUP BY ".$xProj.".chaves2.id ORDER BY chavenum");
        $row0 = pg_num_rows($rs0);
        $pdf->SetFont('Arial', 'I', 14);
        if($Acao == "listamesChavesSoMovimentados"){
...
//            $pdf->ln(2);
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                if($Acao == "listamesChavesSoMovimentados"){
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves2_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('MONTH', datasaida) = '$Mes' And DATE_PART('YEAR', datasaida) = '$Ano'");
                    $rowCont = pg_num_rows($rsCont);
                }
                if($Acao == "listaanoChavesSoMovimentados"){
                    $rs1 = pg_query($Conec, "SELECT TO_CHAR(datasaida, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datavolta, 'DD/MM/YYYY HH24:MI'), TO_CHAR(datasaida, 'YYYY'), TO_CHAR(datavolta, 'YYYY'), usuretira, usudevolve, TO_CHAR(datavolta - datasaida, 'DD HH24:MI'), TO_CHAR(datavolta - datasaida, 'DD'), TO_CHAR(CURRENT_DATE - datasaida, 'DD'), telef, nomedevolve 
                    ...
                    $rsCont = pg_query($Conec, "SELECT id FROM ".$xProj.".chaves2_ctl WHERE ativo = 1 And chaves_id = $Cod And DATE_PART('YEAR', datasaida) = '$Ano'");
                    $rowCont = pg_num_rows($rsCont);
                }
