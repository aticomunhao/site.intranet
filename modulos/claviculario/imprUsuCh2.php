...
//    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
    ...
    if($Acao == "listaUsuarios"){
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE clav2 = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a registrar a Entrega e Devolução de Chaves na DAF:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE clav_edit2 = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a inserir, editar e apagar chaves do claviculário da DAF:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
            $pdf->SetFont('Arial', 'I', 8);
            $pdf->SetX(50);
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE fisc_clav2 = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a fiscalizar o funcionamento do claviculário da DAF:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
            $pdf->SetFont('Arial', 'I', 8);
            $pdf->SetX(50);
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual, cpf FROM ".$xProj.".poslog WHERE chave2 = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Pessoas autorizadas a retirar chaves na DAF:", 0, 'L', false);
        $pdf->ln(5);
        if($row0 > 0){
            $pdf->SetFont('Arial', 'I', 8);
            $pdf->SetX(20);
            ...
            while($tbl0 = pg_fetch_row($rs0)){
                ...
                $rs2 = pg_query($Conec, "SELECT COUNT(usuretira) FROM ".$xProj.".chaves2_ctl WHERE usuretira = $Cod And ativo = 1");
                $tbl2 = pg_fetch_row($rs2);
                $pdf->Cell(20, 5, $tbl2[0], 0, 1, 'R');

                if($EscChave == 1){
                    $rs3 = pg_query($Conec, "SELECT chavenum, chavecompl, chaves_id FROM ".$xProj.".chaves2 INNER JOIN ".$xProj.".chaves2_aut ON ".$xProj.".chaves2.id = ".$xProj.".chaves2_aut.chaves_id 
                    WHERE pessoas_id = $Cod And ".$xProj.".chaves2_aut.ativo = 1 ORDER BY chavenum, chavecompl");
                    ...
                    if($row3 > 0){
                        ...
                        while($tbl3 = pg_fetch_row($rs3)){
                            $CodChave = $tbl3[2];
                            //Conta quantas retiradas de chave
                            $rs4 = pg_query($Conec, "SELECT COUNT(usuretira) FROM ".$xProj.".chaves2_ctl WHERE usuretira = $Cod And ativo = 1 And chaves_id = $CodChave");
