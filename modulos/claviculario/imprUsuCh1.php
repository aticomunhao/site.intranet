...
//    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
    ...
    if($Acao == "listaUsuarios"){
        ...
        $pdf->MultiCell(0, 3, "Usuários autorizados a administrar, editar e fiscalizar o Claviculário da Portaria:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
        ...
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a Registrar Entrega e Devolução de Chaves na Portaria:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
            $pdf->SetFont('Arial', 'I', 8);
            $pdf->SetX(50);
