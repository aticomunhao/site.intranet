    ...
    if($Acao == "devolveChave"){
        ...
        $GuardaCpf = str_replace("-", "", $Cpf2);
         ...
         if($row2 > 0){
            ...
            $rs = pg_query($Conec, "UPDATE ".$xProj.".chaves_ctl SET nomedevolve = '$NomeDevolve', telefdevolve = '$TelefDevolve', datavolta = NOW(), funcrecebe = ". $_SESSION['usuarioID'].", cpfdevolve = '$GuardaCpf', usudevolve = $CodUsu, usuedit = ". $_SESSION['usuarioID'].", dataedit = NOW() WHERE id = $Cod");
         } 
...
//            }
    ...
    if($Acao == "buscausuario"){
        ...
        $rs1 = pg_query($Conec, "SELECT clav, chave, fisc_clav, cpf, clav_edit, nomeusual FROM ".$xProj.".poslog WHERE pessoas_id = $Cod");
        $row1 = pg_num_rows($rs1);
        if($row1 > 0){
            $tbl1 = pg_fetch_row($rs1);
    ...
    if($Acao == "buscacpfusuario"){
        ...
        $rs1 = pg_query($Conec, "SELECT clav, chave, fisc_clav, cpf, pessoas_id, clav_edit, nomeusual FROM ".$xProj.".poslog WHERE cpf = '$GuardaCpf'");
        if(!$rs1){
            $Erro = 1;
            $var = array("coderro"=>$Erro);
        }
    ...
    if($Acao == "configMarcaChave"){
        ...
        if($Campo == "clav_edit" && $Valor == 0){
            $rs = pg_query($Conec, "SELECT id FROM ".$xProj.".poslog WHERE clav_edit = 1");
            $row = pg_num_rows($rs);
            if($row == 1){
                $Erro = 2;
                $var = array("coderro"=>$Erro);
