...
//                $("#relArquivos").load("modulos/arqDaf/conteudo/carRelArq.php?admins="+document.getElementById("admIns").value);
                $("#relArquivos").load("conteudo/carRelArq.php?admins="+document.getElementById("admIns").value);
            ...
            function mostraArq(CodArq){
                ajaxIni();
                if(ajax){
//                   ajax.open("POST", "modulos/arqDaf/conteudo/regconfig.php?acao=selectarquivo&codigo="+CodArq, true);
                   ajax.open("POST", "conteudo/regconfig.php?acao=selectarquivo&codigo="+CodArq, true);
                ...
                $.confirm({
                    ...
                    buttons: {
                        Sim: function () {
                            ajaxIni();
                            if(ajax){
//                                ajax.open("POST", "modulos/arqDaf/conteudo/regconfig.php?acao=apagaarquivo&codigo="+Cod, true);
                                ajax.open("POST", "conteudo/regconfig.php?acao=apagaarquivo&codigo="+Cod, true);
                                ajax.onreadystatechange = function(){
                                    if(ajax.readyState === 4 ){
            ...
            $(document).on('submit', 'form', function(e){
                ...
                if(document.getElementById('arquivo').value !== ""){
                    ...
                    request.addEventListener('load', function(e){
                        ...
                        setTimeout("$('#relArquivos').load('conteudo/carRelArq.php?admins="+document.getElementById('admIns').value+"')", 1000);
                        setTimeout("$form.find('.progress-bar').width( 0 + '%').html( 0 + '%')", 1000); //faz voltar a barra de progresso

                        $('#msg').fadeIn(2000);
                        $('#msg').load('conteudo/carSesArq.php'); // contém a variável de sessão que guarda o sucesso do upload
                        $('#msg').fadeOut(2000);
                        document.getElementById("arquivo").value = "";
                    });
                    request.open("post", "conteudo/arqUpload.php");  //Arquivo responsável em fazer o upload
