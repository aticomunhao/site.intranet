...
if($Acao == "buscausuario"){
    ...
    $rs1 = pg_query($Conec, "SELECT bebed, bebed_fisc, cpf FROM ".$xProj.".poslog WHERE pessoas_id = $Cod");
    $row1 = pg_num_rows($rs1);
    if($row1 > 0){
        $tbl1 = pg_fetch_row($rs1);
        $var = array("coderro"=>$Erro, "bebed"=>$tbl1[0], "fiscbebed"=>$tbl1[1], "cpf"=>$tbl1[2]);
    }else{
        $Erro = 1;
...
if($Acao == "buscacpfusuario"){
    ...
    $rs1 = pg_query($Conec, "SELECT bebed, bebed_fisc, cpf, pessoas_id FROM ".$xProj.".poslog WHERE cpf = '$GuardaCpf'");
    if(!$rs1){
        $Erro = 1;
        $var = array("coderro"=>$Erro);
    }
    ...
    if($row1 > 0){
        $tbl1 = pg_fetch_row($rs1);
        $var = array("coderro"=>$Erro, "bebed"=>$tbl1[0], "fiscbebed"=>$tbl1[1], "cpf"=>$tbl1[2], "PosCod"=>$tbl1[3]);
    }else{
        $Erro = 2;
        $var = array("coderro"=>$Erro);
    }        
...
if($Acao == "buscadadosEquip"){
    $Erro = 0;
    $Cod = (int) filter_input(INPUT_GET, 'codigo');

    $rs0 = pg_query($Conec, "SELECT id, numapar, codmarca, codtipo, codempr, modelo, localinst, observ, TO_CHAR(datatroca, 'DD/MM/YYYY'), TO_CHAR(datavencim, 'DD/MM/YYYY'), TO_CHAR(dataaviso, 'DD/MM/YYYY'), 
...
if($Acao == "configMarcaCheckBox"){
    ...
    if($Campo == "bebed" && $Valor == 0){
        $rs = pg_query($Conec, "SELECT id FROM ".$xProj.".poslog WHERE bebed = 1");
        $row = pg_num_rows($rs);
        if($row == 1){
            $Erro = 2;
            $var = array("coderro"=>$Erro);
...
if($Acao=="buscaMarca"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "SELECT descmarca FROM ".$xProj.".bebed_marcas WHERE id = $Cod And ativo = 1");
    if(!$rs){
        $Erro = 1;
    }
    $tbl = pg_fetch_row($rs);
...
if($Acao=="salvanovaMarca"){
    ...
    $Erro = 0;
    if($Cod > 0){ // salvar
        $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_marcas SET descmarca = '$Nome', usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    }else{ // inserir
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed_marcas");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".bebed_marcas (id, descmarca, usuins, datains) VALUES ($CodigoNovo, '$Nome', ".$_SESSION["usuarioID"].", NOW()) ");
    }
    if(!$rs){
        $Erro = 1;
    }
...
if($Acao=="apagaMarca"){
    ...
    pg_query($Conec, "UPDATE ".$xProj.".bebed SET codmarca = 1, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE codmarca = $Cod ");
    $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_marcas SET ativo = 0, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    if(!$rs){
        $Erro = 1;
    }
    $var = array("coderro"=>$Erro);
...
if($Acao=="buscaTipo"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "SELECT desctipo FROM ".$xProj.".bebed_tipos WHERE id = $Cod And ativo = 1");
    if(!$rs){
        $Erro = 1;
    }
    $tbl = pg_fetch_row($rs);
...
if($Acao=="salvanovoTipo"){
    ...
    $Erro = 0;
    if($Cod > 0){ // salvar
        $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_tipos SET desctipo = '$Nome', usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    }else{ // inserir
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed_tipos");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".bebed_tipos (id, desctipo, usuins, datains) VALUES ($CodigoNovo, '$Nome', ".$_SESSION["usuarioID"].", NOW()) ");
    }
    if(!$rs){
        $Erro = 1;
    }
...
if($Acao=="apagaTipo"){
    ...
    $Erro = 0;
    //tira do arq principal
    pg_query($Conec, "UPDATE ".$xProj.".bebed SET codtipo = 1, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE codtipo = $Cod ");
    $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_tipos SET ativo = 0, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    if(!$rs){
        $Erro = 1;
    }
    $var = array("coderro"=>$Erro);
...
if($Acao=="buscaEmpresa"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "SELECT descempresa, ender, cep, cidade, uf, cnpjempr, inscrempr, telefone, contato, obsempr FROM ".$xProj.".bebed_empr WHERE id = $Cod And ativo = 1");
    if(!$rs){
        $Erro = 1;
    }
    $tbl = pg_fetch_row($rs);
...
if($Acao=="salvaEmpresa"){
    ...
    $Erro = 0;
    if($Cod > 0){ // salvar
        $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_empr SET descempresa = '$Nome', ender = '$editEnder', cep = '$editCEP', cidade = '$editCidade', uf = '$editUF', cnpjempr = '$editCNPJ', inscrempr = '$editInscr', telefone = '$editTelef', contato = '$editContato', obsempr = '$editObs', usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    }else{ // inserir
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed_empr");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".bebed_empr (id, descempresa, ender, cep, cidade, uf, cnpjempr, inscrempr, telefone, contato, obsempr, usuins, datains) 
...
if($Acao == "calcprazo"){
    ...
    $rsCod = pg_query($Conec, "SELECT MAX(numapar) FROM ".$xProj.".bebed");
    $tblCod = pg_fetch_row($rsCod);
    $Codigo = $tblCod[0];
    $NovoFiltro = ($Codigo+1);
...
if($Acao == "salvaEditBebed"){
    $Erro = 0;
    $Cod = (int) filter_input(INPUT_GET, 'codigo'); 
    $NumEquip = (int) filter_input(INPUT_GET, 'numequip'); 
    ...
    if($Cod > 0){ // salvar
        $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed SET numapar = $NumEquip, codmarca = $CodMarca, modelo = '$Modelo', codtipo = $CodTipo, localinst = '$Local', datatroca = '$DataTroca', datavencim = '$DataVenc',
        ...
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".bebed (id, numapar, codmarca, modelo, codtipo, localinst, datatroca, datavencim, dataaviso, prazotroca, diasanteced, observ, notific, usuins, datains) 
        VALUES ($CodigoNovo, $NumEquip, $CodMarca, '$Modelo', $CodTipo, '$Local', '$DataTroca', '$DataVenc', '$DataAviso', '$Prazo', '$DiasAnt', '$Observ', $Notif, ".$_SESSION["usuarioID"].", NOW() ) ");
    }
    if(!$rs){
        $Erro = 1;
    }
...
if($Acao=="apagaEquip"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo');
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed SET ativo = 0 WHERE id = $Cod");
    if(!$rs){
        $Erro = 1;
    }
    $tbl = pg_fetch_row($rs);
...
if($Acao=="mudarAviso"){
    ...
    $Valor = (int) filter_input(INPUT_GET, 'valor');
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed SET pararaviso = $Valor WHERE id = $Cod");
    if(!$rs){
        $Erro = 1;
    }
    $tbl = pg_fetch_row($rs);
...
if($Acao == "insAbastec"){
    $Erro = 0;
    $Cod = (int) filter_input(INPUT_GET, 'codigo'); 
    $DataT = addslashes(filter_input(INPUT_GET, 'data')); 
    $DataTroca = implode("/", array_reverse(explode("/", $DataT)));
    $Volume = (int) filter_input(INPUT_GET, 'volume'); 


        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed_ctl");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1);
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".bebed_ctl (id, bebed_id, datatroca, volume, usuins, datains) 
        VALUES ($CodigoNovo, $Cod, '$DataTroca', $Volume, ".$_SESSION["usuarioID"].", NOW() ) "); 
...
if($Acao == "salvaAbastec"){
    ...
    if($Cod > 0){
        $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_ctl SET datatroca = '$DataTroca', volume =  $Volume, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod "); 
    }else{
        $rsCod = pg_query($Conec, "SELECT MAX(id) FROM ".$xProj.".bebed_ctl");
        $tblCod = pg_fetch_row($rsCod);
        $Codigo = $tblCod[0];
...
if($Acao=="apagaAbastec"){
    $Cod = (int) filter_input(INPUT_GET, 'codigo'); // id de bebed_ctl
    $Erro = 0;
    $rs = pg_query($Conec, "UPDATE ".$xProj.".bebed_ctl SET ativo = 0, usuedit = ".$_SESSION["usuarioID"].", dataedit = NOW() WHERE id = $Cod ");
    if(!$rs){
        $Erro = 1;
    }
    $var = array("coderro"=>$Erro);
