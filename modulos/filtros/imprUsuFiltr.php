...
//    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
    ...
    if($Acao == "listaUsuarios"){
        $pdf->MultiCell(0, 3, "Filtros e Purificadores", 0, 'C', false);
    }
    ...
    if($Acao == "listaUsuarios"){
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE filtros = 1 And ativo = 1 ORDER BY nomecompl");
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE fisc_filtros = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 10);
