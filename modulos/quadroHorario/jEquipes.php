...
<html lang="pt-br">
    ...
    <body> 
        ...
        <div style="padding: 10px;">
            <?php
                $rs3 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual, esc_horaini, esc_horafim, esc_marca FROM ".$xProj.".poslog WHERE esc_eft = 1 And ativo = 1 And esc_grupo = $Cod ORDER BY nomeusual, nomecompl ");
            ?>
            <table class="display" style="margin: 0 auto; width:85%">
                ...
                while($tbl3 = pg_fetch_row($rs3)){
                    ...
                    ?>
                    <tr>
                        <td><input type="checkbox" value="ev" id="ev" title="marca para transferir." onClick="MarcaPartic(<?php echo $Cod ?>);" <?php if($tbl3[5] == 1) {echo "checked";} ?> ></td>
                        ...
                        <td>
                            <select id="buscaturno" onchange="buscaTurno(<?php echo $Cod; ?>, value);" style="font-size: .9rem; width: 24px;" title="Selecione um turno.">
                                <option value="0"></option>
                                <?php 
                                $OpTurnos = pg_query($Conec, "SELECT id, horaini, horafim FROM ".$xProj.".quadroturnos WHERE ativo = 1 ORDER BY horaini, horafim");
                                if($OpTurnos){
