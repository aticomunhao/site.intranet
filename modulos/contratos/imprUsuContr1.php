...
//    $pdf->Cell(0, 5, $Cabec2, 0, 2, 'C');
    ...
    if($Acao == "listaUsuarios"){
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE contr = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a registrar, editar e colecionar os Contratos da Casa:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
        ...
        $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".poslog WHERE fisc_contr = 1 And ativo = 1 ORDER BY nomecompl");
        $row0 = pg_num_rows($rs0);
        $pdf->ln(5);
        $pdf->SetX(20);
        $pdf->SetFont('Arial', 'I', 11);
        $pdf->MultiCell(0, 3, "Usuários autorizados a acompanhar e fiscalizar os Contratos da Casa:", 0, 'L', false);
        $pdf->ln(2);
        if($row0 > 0){
            $pdf->SetFont('Arial', 'I', 8);
            $pdf->SetX(50);
