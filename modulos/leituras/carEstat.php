...
<html lang="pt-br">
    ...
    <body>
        ...
        $rs = pg_query($Conec, "SELECT valorinileitura, TO_CHAR(datainileitura, 'YYYY/MM/DD') FROM ".$xProj.".paramsis WHERE idpar = 1 ");
        $row = pg_num_rows($rs);
        if($row > 0){
            $tbl = pg_fetch_row($rs);
            $ValorIni = $tbl[0];
        ...
        if($row1 > 0){
            while($tbl1 = pg_fetch_row($rs1) ){
                ...
                $SomaLeit3 = 0;
                $SomaLeitAnt = 0;
                $rs2 = pg_query($Conec, "SELECT dataleitura, leitura1, leitura2, leitura3 FROM ".$xProj.".leituras 
                WHERE DATE_PART('MONTH', dataleitura) = $Mes And ativo = 1 And controle = 1 And leitura1 != 0 And leitura2 != 0 And leitura3 != 0 ");
...
//                            $rs3 = pg_query($Conec, "SELECT leitura3 FROM ".$xProj.".leituras 
//                            WHERE DATE_PART('MONTH', dataleitura) = $Mes And dataleitura = (date '$DataLinha' - 1) And ativo = 1 And controle = 1 And leitura1 != 0 And leitura2 != 0 And leitura3 != 0");
                            $rs3 = pg_query($Conec, "SELECT leitura3 FROM ".$xProj.".leituras 
                            WHERE dataleitura = (date '$DataLinha' - 1) And ativo = 1 And controle = 1 And leitura1 != 0 And leitura2 != 0 And leitura3 != 0");
                ...
                <div style="border: 1px solid; border-radius: 10px">
                    <table style="margin: 0 auto; width: 95%;">
                        ...
                        <tr>
                            <td colspan="3" style="border-bottom: 1px solid gray; font-size: 90%; text-align: right;">Consumo Médio Diário: <?php if($QuantDias == 1){echo " (1 dia)";}else{echo "(".$QuantDias." dias)";} ?></td>
                            <td style="border-bottom: 1px solid gray; text-align: center; color: red;"><?php echo number_format($MediaDiaria, 3, ",","."); ?> m<label style="vertical-align: 0.4em; font-size: 70%;">3</label></td>
                        </tr>
                    </table>
                    <br>
                </div>
