...
<html lang="pt-br">
    ...
    <body>
        ...
        ?>
        <div style="text-align: center;"><label class="titRelat corPreta">Controle do Consumo de Eletricidade<?php echo " - ".$Menu2; ?><label></div>
        <?php
        $mes_extenso = array(
            '01' => 'Janeiro',
            '02' => 'Fevereiro',
        ...
        $Condic = "colec = 2 And dataleitura2 IS NOT NULL And leitura2 != 0 And ativo = 1";
        if(isset($_REQUEST["mesano"])){
            ...
            if($MesAno != ""){
                ...
                $Ano = $Proc[1];
                $Condic = "colec = 2 And dataleitura2 IS NOT NULL And leitura2 != 0 And ativo = 1 And DATE_PART('MONTH', dataleitura2) = '$Mes' And DATE_PART('YEAR', dataleitura2) = '$Ano'";
            }else{
                $Condic = "colec = 2 And dataleitura2 IS NOT NULL And leitura2 != 0 And ativo = 1";
            }
        }
        if(isset($_REQUEST["ano"])){
            ...
            if($Ano != ""){
                $Condic = "colec = 2 And dataleitura2 IS NOT NULL And leitura2 != 0 And ativo = 1 And DATE_PART('YEAR', dataleitura2) = '$Ano'";
            }else{
                $Condic = "colec = 2 And dataleitura2 IS NOT NULL And leitura2 != 0 And ativo = 1";
            }
        }
        ...
        if($row1 > 0){
            while($tbl1 = pg_fetch_row($rs1) ){
                ...
                <?php
                    ...
                    $rsY = pg_query($Conec, "SELECT MAX(consdiario2) FROM ".$xProj.".leitura_eletric WHERE ativo = 1 And consdiario2 < 1000");
                    $tblY = pg_fetch_row($rsY);
                    $MaxY = number_format($tblY[0], 0, ",",".");

                    //Preenche arrays para o gráfico
                    $rs0 = pg_query($Conec, "SELECT id, TO_CHAR(dataleitura2, 'DD') as Dia, consdiario2 As Leitura 
                    FROM ".$xProj.".leitura_eletric WHERE ativo = 1 And DATE_PART('MONTH', dataleitura2) = $Mes And DATE_PART('YEAR', dataleitura2) = $Ano And colec = 2 ORDER BY dataleitura2 ");
