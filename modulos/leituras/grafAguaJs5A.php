...
<html lang="pt-BR">
    ...
    <body>
        ...
        $datax2 = [];
        $row2 = 0;
        $rs2 = pg_query($Conec, "SELECT DATE_PART('MONTH', dataleitura), DATE_PART('YEAR', dataleitura), SUM(consdiario) 
        FROM ".$xProj.".leitura_agua 
        WHERE dataleitura IS NOT NULL And leitura1 != 0 And ativo = 1 And DATE_PART('YEAR', dataleitura) = ($AnoAtual-1)  
        GROUP BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura) ORDER BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura)");
        ...
        if($row2 > 0){
            while($tbl2 = pg_fetch_row($rs2) ){
                ...
                if($tbl2[2] > $MaxY){
                    $MaxY = number_format($tbl2[2], 0, ",","."); // Maior valor para o gráfico
                }
            }
        }
        ...
        $datax3 = [];
        $row3 = 0;
        $rs3 = pg_query($Conec, "SELECT DATE_PART('MONTH', dataleitura), DATE_PART('YEAR', dataleitura), SUM(consdiario) 
        FROM ".$xProj.".leitura_agua 
        WHERE dataleitura IS NOT NULL And leitura1 != 0 And ativo = 1 And DATE_PART('YEAR', dataleitura) = ($AnoAtual-2)  
        GROUP BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura) ORDER BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura)");
        ...
        if($row3 > 0){
            while($tbl3 = pg_fetch_row($rs3) ){
                ...
                if($tbl3[2] > $MaxY){
                    $MaxY = number_format($tbl3[2], 0, ",","."); // Maior valor para o gráfico
                }
            }
        }
        ...
        $datax4 = [];
        $row4 = 0;
        $rs4 = pg_query($Conec, "SELECT DATE_PART('MONTH', dataleitura), DATE_PART('YEAR', dataleitura), SUM(consdiario) 
        FROM ".$xProj.".leitura_agua 
        WHERE dataleitura IS NOT NULL And leitura1 != 0 And ativo = 1 And DATE_PART('YEAR', dataleitura) = ($AnoAtual-3)  
        GROUP BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura) ORDER BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura)");
        ...
        if($row4 > 0){
            while($tbl4 = pg_fetch_row($rs4) ){
                ...
                if($tbl4[2] > $MaxY){
                    $MaxY = number_format($tbl4[2], 0, ",","."); // Maior valor para o gráfico
                }
            }
        }
        ...
        $datax5 = [];
        $row5 = 0;
        $rs5 = pg_query($Conec, "SELECT DATE_PART('MONTH', dataleitura), DATE_PART('YEAR', dataleitura), SUM(consdiario) 
        FROM ".$xProj.".leitura_agua 
        WHERE dataleitura IS NOT NULL And leitura1 != 0 And ativo = 1 And DATE_PART('YEAR', dataleitura) = ($AnoAtual-4) 
        GROUP BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura) ORDER BY DATE_PART('YEAR', dataleitura), DATE_PART('MONTH', dataleitura)");
        ...
        if($row5 > 0){
            while($tbl5 = pg_fetch_row($rs5) ){
                ...
                if($tbl5[2] > $MaxY){
                    $MaxY = number_format($tbl5[2], 0, ",","."); // Maior valor para o gráfico
                }
            }
        }
        ...
        <script>
            ...
            layout = {
                ...
                xaxis: {range: [1, 12], title: "Mês/Ano"},
