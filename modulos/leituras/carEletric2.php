    ...
    <body>         
        <?php 
            ...
            $admEdit = parAdm("editleituraeletric", $Conec, $xProj); // nível para editar
             ...
             $rsDia1 = pg_query($Conec, "SELECT MAX(dataleitura2) FROM ".$xProj.".leitura_eletric WHERE colec = 2 And ativo = 1 ");
            ...
            if(isset($_REQUEST["mesano"])){
                ...
                if($MesAno != ""){
                    ...
                    $Ano = $Proc[1];
                    $Condic = "colec = 2 And ativo = 1 And DATE_PART('MONTH', dataleitura2) = '$Mes' And DATE_PART('YEAR', dataleitura2) = '$Ano'";
                }else{
                    $Condic = "colec = 2 And ativo = 1";
                }
            }
            if(isset($_REQUEST["ano"])){
                ...
                if($Ano != ""){
                    $Condic = "colec = 2 And ativo = 1 And DATE_PART('YEAR', dataleitura2) = '$Ano'";
                }else{
                    $Condic = "colec = 2 And ativo = 1";
                }
            }
            $rs0 = pg_query($Conec, "SELECT id, TO_CHAR(dataleitura2, 'DD/MM/YYYY'), date_part('dow', dataleitura2), leitura2, dataleitura2 FROM ".$xProj.".leitura_eletric WHERE $Condic ORDER BY dataleitura2 DESC ");
            $Leit24Ant = 0;
            $Cont = 0;
            ?>
            <div  style="text-align: center;"><label class="titRelat corPreta">Leituras Medidor Eletricidade da Operadora<?php echo " - ".$Menu2; ?><label></div>
