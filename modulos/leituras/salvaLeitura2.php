...
if($Acao =="salvaDataEletric"){
    ...
    if($Cod == 0){
        ...
        $Codigo = $tblCod[0];
        $CodigoNovo = ($Codigo+1); 
        $rs = pg_query($Conec, "INSERT INTO ".$xProj.".leitura_eletric (id, colec, dataleitura2, leitura2, ativo, usuins, datains) 
        VALUES($CodigoNovo, $Colec, '$PegaDia', $Leitura2, 1, ".$_SESSION["usuarioID"].", NOW() )");
        if(!$rs){
            $Erro = 1;
        }
        ...
        $rs = pg_query($Conec, "UPDATE ".$xProj.".leitura_eletric SET colec = $Colec, dataleitura2 = '$PegaDia', leitura2 = $Leitura2, usumodif = ".$_SESSION["usuarioID"].", datamodif = NOW() WHERE id = $Cod ");
        if(!$rs){
            $Erro = 1;
        }
    }
