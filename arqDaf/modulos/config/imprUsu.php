...
if(isset($_REQUEST["acao"])){
    ...
    $rsCabec = pg_query($Conec, "SELECT cabec1, cabec2, cabec3 FROM ".$xProj.".daf_setores WHERE codset = ".$_SESSION["CodSetorUsuDaf"]." ");
...
//    $pdf->SetX(40); 
    ...
    $rs0 = pg_query($Conec, "SELECT id FROM ".$xProj.".daf_poslog WHERE ativo = 1");
    ...
    if($Acao == "listaUsuarios"){
        ...
        if($row > 0){
            while($tbl = pg_fetch_row($rs)){
                ...
                $rs0 = pg_query($Conec, "SELECT pessoas_id, nomecompl, nomeusual FROM ".$xProj.".daf_poslog WHERE adm = $Adm And ativo = 1 ORDER BY nomeusual, nomecompl");
                $row0 = pg_num_rows($rs0);
